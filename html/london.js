...
window.addEventListener("load", function (event) {
    ...
    const url = "http://ws.audioscrobbler.com/2.0/?method=geo.gettoptracks&country=united+kingdom&api_key=a4639d1359fd1cae45a045b4b7114178&limit=3&format=json";
    fetch(url)
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
        var londonTrack = data.tracks.track.map((track) => {
          return `<div>
              <ol>
        ...
        document.getElementById("data").innerHTML = londonTrack;
      });
  });
...
function toggleModal() {
  const modalOverlay = document.querySelector('#modal1');
  modalOverlay.toggle('visible');
}
// CLOSE MODAL
